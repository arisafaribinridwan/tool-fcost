...
const App = () => {
  // State Utama
  const [appState, setAppState] = useState('idle'); // idle, ready, running, success
  ...
  const jobs = {
    'job-01': {
      name: 'Transformasi Sales Bulanan',
      workflow: 'sales_transformation_v2.yaml',
      template: 'template_sales_report_2024.xlsx'
    },
    'job-02': {
      name: 'Sinkronisasi Stok Gudang',
      workflow: 'warehouse_sync_daily.yaml',
      template: 'stock_reconciliation_template.xlsx'
    },
    'job-03': {
      name: 'Format Laporan Keuangan',
      workflow: 'finance_reporting_monthly.yaml',
      template: 'financial_statement_template.xlsx'
    }
  };

  const activeJob = jobs[selectedJob];
  ...
  const handleFileSelect = () => {
    ...
    setAppState('ready');
    addLog('info', `Job aktif: ${jobs[selectedJob].name}`);
  };
  ...
  const handleClearSourceFile = () => {
    ...
    setProgress(0);
    setAppState('idle');
    addLog('info', 'File sumber dilepas dari sesi aktif.');
  };
              ...
              <div className="flex-1 flex flex-col justify-center gap-4">
                {!sourceFile ? (
                  <button 
                    ...
                    className="group border-2 border-dashed border-slate-200 rounded-xl p-8 md:p-10 hover:border-indigo-400 hover:bg-indigo-50/50 transition-all text-center"
                  >
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={handleClearSourceFile} className="text-slate-300 hover:text-rose-500">
                        <X size={16} />
                      </button>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Job Aktif</p>
                    <p className="mt-1 text-sm font-bold text-slate-700">{activeJob.name}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Workflow</p>
                    <p className="mt-1 text-sm font-bold text-slate-700 truncate">{activeJob.workflow}</p>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Template</p>
                    <p className="mt-1 text-sm font-bold text-slate-700 truncate">{activeJob.template}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-4">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Workflow Aktif</p>
                    <p className="mt-1 text-sm font-bold text-slate-800">{activeJob.workflow}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Template Output</p>
                    <p className="mt-1 text-sm font-bold text-slate-800">{activeJob.template}</p>
                  </div>
                  <p className="text-xs leading-relaxed text-slate-500">
                    Konfigurasi aktif dipakai langsung saat job dijalankan agar alur utama lebih ringkas.
                  </p>
                </div>
              </div>
            </div>
