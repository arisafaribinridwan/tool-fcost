...
Menambahkan kolom `prod_month` berdasarkan ekstraksi karakter dari `serial_number`.
...
- Nama `prod_month` dipertahankan dulu agar konsisten dengan flow awal, walaupun secara semantik ini lebih dekat ke `prod_lot_code`.
- Hasil yang ditulis adalah string statis final.

### Bentuk Pseudocode

```text
prod_month = serial_number[5:8]
```
...
- `serial_number` tersedia untuk `prod_month`
...
- `prod_month` mengikuti ekstraksi 3 karakter dari `serial_number`
